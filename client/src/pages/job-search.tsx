    ...
    <AppLayout>
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-6 relative">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm" style={{ color: '#737373' }}>Browse opportunities and get referred for Rs.499</p>
        </div>

        {/* ── Search ──────────────────────── */}
        <div className="relative mb-4">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5" style={{ color: '#737373' }} />
          ...
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 rounded flex items-center justify-center"
              style={{ background: '#1C1C1C', color: '#737373' }}><X className="w-3 h-3" /></button>
          )}
        </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-6">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
                  ...
                  style={{ background: '#141414', border: '1px solid #1F1F1F', color: '#D4D4D4' }}>Prev</button>
                ...
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
                  className="h-8 px-3 rounded-lg text-xs font-bold disabled:opacity-30 transition-all"
                  style={{ background: '#141414', border: '1px solid #1F1F1F', color: '#D4D4D4' }}>Next</button>
              </div>
            )}
          </motion.div>
        ) : (
          <div className="rounded-2xl p-12 text-center relative overflow-hidden" style={{ background: '#141414', border: '1px solid #1F1F1F' }}>
            ...
            <div className="relative z-10">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: '#1C1C1C' }}>
                <Search className="w-7 h-7" style={{ color: '#737373' }} />
              </div>
              <p className="text-base font-bold mb-1" style={{ color: '#F5F5F5' }}>No jobs found</p>
              <p className="text-sm mb-5" style={{ color: '#737373' }}>Try adjusting your search or filters</p>
